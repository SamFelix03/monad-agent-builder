...
export default function NodeLibrary() {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <h2 className="mb-3 text-base font-semibold tracking-tight">Workflow Library</h2>
      {toolTypes.map((tool) => (
        <Button
          key={tool.type}
          variant="outline"
          className="h-auto min-h-14 justify-start rounded-xl border-border/60 bg-card/50 py-3 text-left transition-spring hover:border-[color-mix(in_oklch,var(--brand)_30%,var(--border))]"
          draggable={true}
